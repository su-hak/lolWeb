...
public class ProbuildController {
    ...
    private List<LeagueEntryDTO> getAllLeagueEntries(String queue, String tier, String division) {
        // 먼저 전체 데이터를 가져옵니다.
        int all = Integer.MAX_VALUE; // 제한 없이 모두 가져오기 위해 최대값 설정
        int limit = 20; // 50개만 가져오기

        int page = 0; // 페이지는 0으로 설정하여 한 번에 모든 데이터를 가져옵니다.
        Page<LeagueEntryDTO> leagueEntriesPage = probuildService.getLeagueInfo(queue, tier, division, all, page);
